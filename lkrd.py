...
desc: 仅限安卓机使用， 每天1块，多号多薅
      下载地址 http://www.1y2y.com/game/191813.html
"""
# https://apilkrd.cengaw.cn/api/XXXX  请求头 deviceId#Authorization
# 变量 lkrdck, 多变量换行分割
import random
import sys
...
lkrdCks = os.environ.get("lkrdck") if os.environ.get(
    "lkrdck") else "27ec497509f72825#eyJ0eXAiOiJKV1MiLCJhbGciOiJIUzUxMiJ9.eyJleHAiOjE5OTgzNjAzMTMsImlhdCI6MTY4MzAwMDMxMywibmJmIjoxNjgzMDAwMzEzLCJqdGkiOiI2NDUwOGJmOWYwMzI5Iiwic3ViIjoxMTY2MTQsImlzcyI6Imh0dHA6XC9cL2FwaWxrcmQuY2VuZ2F3LmNuIiwiYXVkIjoiaHR0cDpcL1wvYXBpbGtyZC5jZW5nYXcuY24ifQ.OTgzYzNmNDljMjQwMmFhZmNmODJmMGE2MDYxY2Y0YzlkMWM1NzIwNTk0MmE0Mjg5MjAxMjdiZmZiMGQxNGI0M2Y5NzYxZDViYTIwZDliYTVmYzRhOTQyYWY4NDI4NjU0NDY5Y2I3M2Q2MjdlNzJmODg0ZTNmNzZiYWU0YjE3Mjc"
lkrdCkList = lkrdCks.split("\n")
if len(lkrdCkList) == 0 or lkrdCks == '':
    print('未填写乐看热点环境变量：lkrdck')
    sys.exit()
...
def loadAd(authorization, deviceId, type, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    ...
    url = f'https://apilkrd.cengaw.cn/api/v2/ads/action/load?class={class1}&&channel={channel}&type={type}'
...
def showedAd(authorization, deviceId, type, tid, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    class1, channel, type, platformname = getParam(type)
    ecpm = '6000.0'
    url = f'https://apilkrd.cengaw.cn/api/v2/ads/action/showed?class={class1}&channel={channel}&type={type}&ecpm={ecpm}&tid={tid}&platformname={platformname}'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.get(url=url, headers=headers, timeout=20, proxies=proxy)
    res = resp.json()
    if res['success']:
...
def clickedAd(authorization, deviceId, type, tid, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    class1, channel, type, platformname = getParam(type)
    ecpm = '6000.0'
    url = f'https://apilkrd.cengaw.cn/api/v2/ads/action/clicked?class={class1}&channel={channel}&type={type}&ecpm={ecpm}&tid={tid}&platformname={platformname}'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.get(url=url, headers=headers, timeout=20, proxies=proxy)
    res = resp.json()
    if res['success']:
...
def compeletedAd(authorization, deviceId, type, tid, ticket='', proxy=None, proxyAuthorization=None, proxy_type='xk'):
    class1, channel, type, platformname = getParam(type)
    ecpm = '6000.0'
    url = f'https://apilkrd.cengaw.cn/api/v2/ads/action/completed?class={class1}&type={type}&ticket={ticket}&ecpm={ecpm}&tid={tid}&platformname={platformname}'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.get(url=url, headers=headers, timeout=20, proxies=proxy)
    res = resp.json()
    if res['success']:
...
def initAd(authorization, deviceId, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    url = f'https://apilkrd.cengaw.cn/api/v2/ads/action/init'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.get(url=url, headers=headers, timeout=20, proxies=proxy)
    res = resp.json()
    if res['success']:
...
def getBubble(authorization, deviceId, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    url = f'https://apilkrd.cengaw.cn/api/v2/reward/bubble'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.get(url=url, headers=headers, timeout=20, proxies=proxy)
    res = resp.json()
    if res['success']:
...
def getBubble2(authorization, deviceId, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    url = f'https://apilkrd.cengaw.cn/api/v2/reward/bubble2'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.get(url=url, headers=headers, timeout=20, proxies=proxy)
    res = resp.json()
    if res['success']:
...
def signState(authorization, deviceId, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    url = f'https://apilkrd.cengaw.cn/api/v2/reward/sign'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.get(url=url, headers=headers, timeout=20, proxies=proxy)
    res = resp.json()
    if res['success']:
...
def getRedRain(authorization, deviceId, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    url = f'https://apilkrd.cengaw.cn/api/v2/reward/rain'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.get(url=url, headers=headers, timeout=20, proxies=proxy)
    res = resp.json()
    if res['success']:
...
def getMyInfo(authorization, deviceId, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    url = f'https://apilkrd.cengaw.cn/api/v2/member/profile'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.get(url=url, headers=headers, timeout=20, proxies=proxy)
    try:
        res = resp.json()
...
def getZuanTicket(authorization, deviceId, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    url = f'https://apilkrd.cengaw.cn/api/v2/zhuan/video'
    payload = f"type=1"
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
...
def getZuanIndex(authorization, deviceId, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    url = f'https://apilkrd.cengaw.cn/api/v2/zhuan/index'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.post(url=url, headers=headers, timeout=20, proxies=proxy)
    res = resp.json()
    if res['success']:
...
def newsCost(authorization, deviceId, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    url = f'https://apilkrd.cengaw.cn/api/v2/news/cost'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.get(url=url, headers=headers, timeout=20, proxies=proxy)
    res = resp.json()
    if res['success']:
...
def newsCoin(authorization, deviceId, ticket='', proxy=None, proxyAuthorization=None, proxy_type='xk'):
    if ticket == '':
        url = f'https://apilkrd.cengaw.cn/api/v2/news/coin'
    else:
        url = f'https://apilkrd.cengaw.cn/api/v2/news/coin?ticket={ticket}'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.get(url=url, headers=headers, timeout=20, proxies=proxy)
    res = resp.json()
    # print(str(res))
...
def videoCost(authorization, deviceId, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    url = f'https://apilkrd.cengaw.cn/api/v2/video/cost?type=short'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.get(url=url, headers=headers, timeout=20, proxies=proxy)
    res = resp.json()
    if res['success']:
...
def videoCoin(authorization, deviceId, ticket='', proxy=None, proxyAuthorization=None, proxy_type='xk'):
    if ticket == '':
        url = f'https://apilkrd.cengaw.cn/api/v2/video/coin'
    else:
        url = f'https://apilkrd.cengaw.cn/api/v2/video/coin?ticket={ticket}'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.get(url=url, headers=headers, timeout=20, proxies=proxy)
    res = resp.json()
    # print(str(res))
...
def redEnv(authorization, deviceId, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    url = f'https://apilkrd.cengaw.cn/api/v2/news/redenv'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.get(url=url, headers=headers, timeout=20, proxies=proxy)
    res = resp.json()
    if res['success']:
...
def redVideoEnv(authorization, deviceId, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    url = f'https://apilkrd.cengaw.cn/api/v2/video/redenv'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.get(url=url, headers=headers, timeout=20, proxies=proxy)
    res = resp.json()
    if res['success']:
...
def done(authorization, deviceId, id, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    url = f'https://apilkrd.cengaw.cn/api/v2/zhuan/done'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    payload = f'id={id}'
...
def newsCount(authorization, deviceId, isOpen, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    url = f'https://apilkrd.cengaw.cn/api/v2/news/sdk/zhuan/count?isfirstopen={isOpen}'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.get(url=url, headers=headers, timeout=20, proxies=proxy)
    res = resp.json()
    if res['success']:
...
def getKanList(authorization, deviceId, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    url = f'https://apilkrd.cengaw.cn/api/v2/kan/index?page=1&per_page=200'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.get(url=url, headers=headers, timeout=20, proxies=proxy)
    res = resp.json()
    if res['success']:
...
def clickKan(authorization, deviceId, id, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    url = f'https://apilkrd.cengaw.cn/api/v2/kan/click?id={id}'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.get(url=url, headers=headers, timeout=20, proxies=proxy)
    res = resp.json()
    if res['success']:
...
def getKanPrize(authorization, deviceId, id, ticket, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    url = f'https://apilkrd.cengaw.cn/api/v2/kan/index'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
...
def exchange(authorization, deviceId, proxy=None, proxyAuthorization=None, proxy_type='xk'):
    url = f'https://apilkrd.cengaw.cn/api/v2/cash/exchange'
    headers = getHeader(authorization, deviceId, proxyAuthorization, proxy_type)
    resp = requests.get(url=url, headers=headers, timeout=20, proxies=proxy)
    res = resp.json()
    if res['success']:
...
if __name__ == '__main__':
    ...
    try:
        random.shuffle(lkrdCkList)
        for lkrdCK in lkrdCkList:
            model = models[random.randint(0, len(models) - 1)]
            print("随机等待60-100秒")
            time.sleep(random.randint(60, 100))
            deviceId = lkrdCK.split("#")[0]
            authorization = lkrdCK.split("#")[1]
