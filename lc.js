...
!(async () => {
    ...
    if ($.message) {
        console.log(`\n==========运行完成==========\n`)
        console.log($.message)
        await notify.sendNotify("仑传新闻-APP", $.message)
    }
})()
    .finally(() => {
        $.done();
...
function credential_auth(phone, pwd) {
    ...
    let headers = {
        "User-Agent": "ANDROID;10;10014;5.0.0;1.0;null;Redmi Note 7 Pro",
    ...
    let body = `client_id=17&password=${encodeURIComponent(encryptPwd)}&phone_number=${phone}`
    let postRequest = {
        url: url,
        method: "POST",
        headers: headers,
...
function login(authCode, sessionId) {
    ...
    let raw = `${path}&&${sessionId}&&${requestId}&&${now}&&FR*r!isE5W&&18`
...
function getArticles(sessionId) {
    let list_count = parseInt(Math.random() * 5) * 10
    console.log(`✅ 获取文章列表list_count为${list_count}`)
    let path = '/api/article/channel_list'
    let requestId = generateRequestId().toUpperCase()
    let now = moment().valueOf()
    let raw = `${path}&&${sessionId}&&${requestId}&&${now}&&FR*r!isE5W&&18`
    let sign = encryptUtil.sha256Decrypt(raw)
    let url = `https://vapp.tmuyun.com${path}?channel_id=5cdcc989b198504a785efad1&isDiangHao=false&is_new=true&list_count=${list_count}&size=10`
    let headers = {
        "X-SESSION-ID": sessionId,
        "User-Agent": "ANDROID;10;10014;5.0.0;1.0;null;Redmi Note 7 Pro",
        "X-REQUEST-ID": requestId,
...
function read(sessionId, articleId) {
    ...
    let raw = `${path}&&${sessionId}&&${requestId}&&${now}&&FR*r!isE5W&&18`
    let sign = encryptUtil.sha256Decrypt(raw)
    let url = `https://vapp.tmuyun.com${path}?id=${articleId}&tenantId=18&url_Path=/webDetails/news`
    let headers = {
        "X-SESSION-ID": sessionId,
        "User-Agent": "ANDROID;10;10014;5.0.0;1.0;null;Redmi Note 7 Pro",
        "X-REQUEST-ID": requestId,
...
function comment(sessionId, articleId, txt) {
    ...
    let body = `channel_article_id=${articleId}&content=${encodeURIComponent(txt)}`
    let getRequest = {
        "url": url,
        "headers": headers,
        "body": body,
