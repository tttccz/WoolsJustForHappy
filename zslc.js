...
let sid = 28
let clientId = 43
let channelId = '5d5361fb1b011b137b853d34'
...
!(async () => {
    ...
    if ($.message) {
        console.log(`\n==========运行完成==========\n`)
        console.log($.message)
        await notify.sendNotify("掌上鹿城APP", $.message)
    }
})()
    .finally(() => {
        $.done();
...
function getArticles(sessionId) {
    ...
    let raw = `${path}&&${sessionId}&&${requestId}&&${now}&&FR*r!isE5W&&${sid}`
    let sign = encryptUtil.sha256Decrypt(raw)
    let url = `https://vapp.tmuyun.com${path}?channel_id=${channelId}&is_new=1&list_count=${list_count}&size=10`
    let headers = {
        "X-SESSION-ID": sessionId,
        "User-Agent": "ANDROID;10;62;5.0.0;1.0;null;Redmi Note 7 Pro",
        "X-REQUEST-ID": requestId,
