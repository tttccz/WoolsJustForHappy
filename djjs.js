...
let sid = 24
let clientId = 35
let channelId = '5d3fe981b198500f695bdebf'
...
!(async () => {
    ...
    if ($.message) {
        console.log(`\n==========运行完成==========\n`)
        console.log($.message)
        await notify.sendNotify("多娇江山-APP", $.message)
    }
})()
    .finally(() => {
        $.done();
...
function getArticles(sessionId) {
    ...
    let raw = `${path}&&${sessionId}&&${requestId}&&${now}&&FR*r!isE5W&&${sid}`
    let sign = encryptUtil.sha256Decrypt(raw)
    let url = `https://vapp.tmuyun.com${path}?channel_id=${channelId}&isDiangHao=false&is_new=true&list_count=${list_count}&size=10`
    let headers = {
        "X-SESSION-ID": sessionId,
        "User-Agent": "ANDROID;10;62;5.0.0;1.0;null;Redmi Note 7 Pro",
        "X-REQUEST-ID": requestId,
